...
public class DashController implements Initializable {
    ...
    @FXML
    private TableColumn<?, ?> btype;
    @FXML
    private TableColumn<?, ?> complainant;
    @FXML
    private TableColumn<?, ?> respondent;
    @FXML
    private TableColumn<?, ?> victim;
    @FXML
...
"WHERE household.headoffamily=person.id AND lname=? ";
    ...
    private void newBlotterHandler(ActionEvent event) throws IOException { //new blotter
        Parent blah = FXMLLoader.load(getClass().getResource("/fxml/Blotter.fxml"));
        Scene scene = new Scene(blah);
        Stage stage = new Stage();
        stage.setScene(scene);
        stage.centerOnScreen();
...
//            String query2 = "SELECT * FROM person WHERE id=?";
    ...
    private void deleteperson(ActionEvent event) throws IOException, SQLException {
        Person person0 = tableviewperson.getSelectionModel().getSelectedItem();
        if(DialogController.alertconfirm("Delete Person", "Are you sure you want to delete this person? ",person0.getNAME() +" [" + person0.getId()+"]")) {
